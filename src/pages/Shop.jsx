...
export default function Shop() {
  ...
  // إظهار المقاسات فقط عند اختيار "الكل" أو "الصنادل"
  const showSizes = selectedCategory === '' || selectedCategory === 'sandales';
  ...
  // عند اختيار Sacs، مسح المقاس المختار
  useEffect(() => {
    if (!showSizes) {
      setSelectedSize('');
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center justify-between mb-6 gap-4">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-white border border-gray-300 rounded-xl px-4 py-2 pr-8 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500"
            >
        ...
        {showFilters && (
          <div className="bg-white rounded-2xl p-6 mb-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              {hasFilters && (
                <button onClick={clearFilters} className="text-sm text-rose-600 hover:text-rose-700 flex items-center gap-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="category" checked={!selectedCategory} onChange={() => setSelectedCategory('')} className="text-rose-600 focus:ring-rose-500" />
                    <span className="text-sm">Toutes</span>
                  </label>
                  {categories.map(cat => (
                    <label key={cat.id} className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="category" checked={selectedCategory === cat.slug} onChange={() => setSelectedCategory(cat.slug)} className="text-rose-600 focus:ring-rose-500" />
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className={`w-8 h-8 rounded-lg border-2 transition-all flex items-center justify-center ${!selectedColor ? 'border-rose-600 shadow-md scale-110' : 'border-gray-200 hover:border-rose-300'}`}
                    title="Toutes"
                  >
                    <span className="text-[8px] text-gray-400 font-bold">T</span>
                  </button>
                  {COLORS.map(c => (
                    <button
                      key={c}
                      onClick={() => setSelectedColor(selectedColor === c ? '' : c)}
                      className={`w-8 h-8 rounded-lg border-2 transition-all relative ${selectedColor === c ? 'border-rose-600 shadow-md scale-110' : 'border-gray-200 hover:border-rose-300 hover:scale-105'}`}
                      title={c}
                    >
                      <div
                      ...
                      {selectedColor === c && (
                        <div className="absolute inset-0 flex items-center justify-center">
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={c === 'Blanc' || c === 'Beige' ? '#333' : '#fff'} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
              ...
              {/* Size — يظهر عند اختيار الكل أو الصنادل */}
              {showSizes && (
                <div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <button onClick={clearFilters} className="mt-4 text-rose-600 hover:text-rose-700 font-medium">Réinitialiser les filtres</button>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 pb-12">
